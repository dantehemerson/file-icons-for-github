...
- Reads the Document Object Model (DOM) of those pages to locate file names
  so it can render an icon before each file name.
...
The extension requests no `permissions` entries. Its `host_permissions`
(Chromium) / `permissions` (Firefox) are limited to:

- `https://github.com/*`
- `https://gist.github.com/*`

This is required so the extension can read and modify the page for the sole
purpose of displaying icons. All processing happens locally in your browser.
